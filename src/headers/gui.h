...
class Gui {
 public:
  ...
  static GtkWidget* login_window_;
 ...
 private:
  /**
 * @brief Determines if the add item entries are filled and adjusts the save button sensitivity.
 *
 * This function is designed to be called as a callback for a GtkEditable widget.
 * It checks if the headline, username, and password entry fields are filled, and adjusts
 * the sensitivity of the save button accordingly.
 *
 ...
 * @param user_data A gpointer, but not used in this function.
  ...
  /**
 * @brief Determines if the login entries are filled and adjusts the save button sensitivity.
 *
 * This function is designed to be called as a callback for a GtkEditable widget.
 * It checks if the username and password entry fields are filled, and adjusts the
 * sensitivity of the save button accordingly.
 *
 * @param object The GtkEditable instance that triggered the callback.
 * @param user_data A gpointer, but not used in this function.
   */
 ...
 *
 * This function is designed to be called as a callback for a GtkButton widget.
 * It creates a new "Add Item" modal window, which includes fields for entering
 * headline, username, and password, as well as buttons for generating a password
 * and saving the new item.
 *
 ...
 */
  ...
  /**
 * @brief Adds a new item to the password list and saves it to the UserDatabase.
 *
 * This function is designed to be called as a callback for a GtkButton or another
 * GtkWidget. It retrieves the input values from the entry fields, creates a new
 * Record object, and adds it to the UserDatabase. The function also updates the
 * password list UI with the newly created item.
 *
 ...
 *
 * This function is called when the application is activated. It is responsible
 * for showing the login window of the RainText application.
 *
 ...
 *
 * This function sets up the main window, including the layout and widgets, and
 * connects the appropriate signals to their corresponding callback functions.
 *
 * @param app The GtkApplication instance the main window is associated with.
*/
 ...
 * library and sets the text of the GtkPasswordEntry to the generated password.
 *
 * @param button The GtkButton instance that triggered the callback.
 * @param user_data A gpointer pointing to the GtkPasswordEntry widget where the
 * generated password should be set.
   */
 ...
 *
 * This function takes a Record as input and creates a GtkWidget containing the
 * record's data. The resulting widget includes labels for the headline,
 * username, and a masked password, as well as buttons for editing and deleting
 * the record.
 *
...
/**
 * @brief Switches between the login and registration pages in the user interface.
 *
 * This function toggles the visibility of the login and registration pages in
 * the user interface using a GtkStack. When the button is clicked, the function
 * checks the currently visible child and switches to the other child.
 *
 * @param button A pointer to the GtkButton that triggered the function.
 * @param user_data A gpointer holding the reference to the GtkStack containing the login and registration pages.
*/
